...
const ProblemView = ({ noteId, isCurrentCard = false, onRefresh, onNavigateBack }: ProblemViewProps) => {
  ...
  const fetchProblemData = async () => {
    ...
    // 過去の解答エントリを取得 (data.noteInfo が存在する場合のみ)
    // Check if 'noteInfo' exists in data before accessing its properties
  ...
  return (
    <div className="container mx-auto">
      ...
      {/* Pass error message string to renderError */}
      {renderError(noteError ? noteError.message : null)}
      {renderError(mediaError ? mediaError.message : null)}
